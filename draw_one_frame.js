...
function draw_one_frame(cur_frac) {
	...
	for(let accross = -1; accross <= width*1.6 /spacingSize; accross++ ){
		...
		push();
		rectMode(CENTER);
		let top_rotSquare = map(cur_frac, firstStop,0.5, 0, -22.5); 
		let top_rotQuad = map(cur_frac,secondStop,1, -22.5, 0);
		let top_deformSquare = map(cur_frac,firstStop,0.5,0, 45);
		let top_deformQuad = map(cur_frac,secondStop,1,45,0);
		let top_scaleSquare = map(cur_frac,firstStop,0.5,1,0.5);
		let top_scaleQuad = map(cur_frac,secondStop,1,0.5,1);
